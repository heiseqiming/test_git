...
The following theorem tells us how to express the area of S as an integral.
...
G = {$(x, y)$|a<x<b,0<y<g(x)}.
\]
That is, G is the ordinate set of g, and F is the ordinate set of f, minus the graph of f. The region S between the graphs of f and g is the difference S = G — F. By Theorems 1.10 and 1.11, both F and G are measurable. Since F ⊂ G, the difference S = G — F is also measurable, and we have

a(S) = a(G) - a(F) = P g(x) dx - Pf(x) dx = P [g(x) -f(x)] dx .
